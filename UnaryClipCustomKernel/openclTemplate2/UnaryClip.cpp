...
//从磁盘加载内核源文件并创建一个程序对象
cl_program CreateProgram(cl_context context, cl_device_id device, string typeName)
{
	cl_int errNum;
	cl_program program;
	...
	*/
	string src = "__kernel void UnaryClipCustomKernel(const int size_in, \
                                     __global const " + typeName + " * in0, \
                                     __global const " + typeName + " * in1, \
                                     __global const " + typeName + " * in2, \
                                     __global " + typeName + " * out){ \
    int index = get_global_id(0); \
    if(index < size_in){ \
        " + typeName + "  value = in2[0] < in0[index] ? in2[0] : in0[index]; \
...
int main(int argc, char** argv)
{
	...
	string typeName = getTypeName(in0[0]);
	program = CreateProgram(context, device, typeName);
	if (program == NULL)
	{
		Cleanup(context, commandQueue, program, kernel, memObjects);
		system("pause");
